...
var blink1_setColor = function(r,g,b, fadeMillis) { 
  ...
  blink1Device.controlTransferIn(blink1Device, transferInfo)
}
...
requestPermission.addEventListener('click', function() {
  // navigator.permissions.request( permissionObj, function(result) {
  //   if (result) {
  //     gotPermission();
  //   } else {
  ...
  //   }
  // });
  navigator.usb.requestDevice({ filters: [{ vendorId: 10168 }] })
  .then(device => {
    blink1Device = device;
    gotPermission();
    blink1_setColor(155, 0, 0, 70)
...
// navigator.permissions.contains( permissionObj, function(result) {
//   if (result) {
//     gotPermission();
//   }
// });
